...
typedef struct path {
  heap_node_t *hn;
  //hold the position within the world
  uint8_t pos[2];
  //can remove this field for 1.03
  uint8_t from[2];
  //cost of paththrough this node
  int32_t cost;
} path_t;
...
//pointer for a map of 401x401
map_t* world_map[401][401];
int north;
int south;
int east;
int west;
...
int main(int argc, char *argv[])
{
  ...
  //taking input from the user and keeping track of the coordinates
  char c;
	int x = 200;
	int y = 200;
  ...
  //create initial map
  world_map[x][y] = malloc(sizeof (map_t));
  new_map(world_map[x][y]);
  place_pokemart(world_map[x][y]);
  place_center(world_map[x][y]);  
  print_map(world_map[x][y]);

  while (c != 'q')
  {
    //randomize the chances of a pokeCenter and pokeMart to appear
    int pokeC_prob = rand() % 100;
    int pokeM_prob = rand() % 100;
    //if the random number is <= 50 then place a pokeM or pokeC
    //this is to increase the difficulty of find a mart or center
	  ...
	  switch (c) {
        ...
        print_map(world_map[x][y]);     
		  ...
		  default:
			  // fprintf(stderr, "Error, invalid input: %c\n", c); //this line for debugging
			  fprintf(stderr, "Enter n, s, e, w, f, or q\n"); //for players
	  }
  }
