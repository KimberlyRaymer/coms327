...
void io_pokemon_center()
{
  ...
  // // for (int i = 0; i < 6; i++) {
  // world.pc.buddy[0]->reset_hp(world.pc.buddy[0]->get_ogHP());
  // // }
  for (int i = 0; i < 6 && world.pc.buddy[i]; i++)
  {
    world.pc.buddy[i]->reset_hp(world.pc.buddy[i]->get_ogHP());
    printw(", %s", world.pc.buddy[i]->get_species());
  }
  refresh();
  getch();
}
...
void io_pokemonBattle(pokemon *p)
{
  ...
  pokemon *pc_poke = world.pc.buddy[n_pokeIdx];
  ...
  do
  {
    ...
    {
      n_pokeIdx++;
      if (n_pokeIdx > numPoke)
        inBattle = 0;

      else if (world.pc.buddy[n_pokeIdx] == NULL)
        inBattle = 0;

      else
        pc_poke = world.pc.buddy[n_pokeIdx];
    }
    else if (p->get_hp() == 0)
    {
      inBattle = 0;
    ...
    mvprintw(0, 0, "A wild %s appeared! \nHP:%d", p->get_species(), p->get_hp());
    mvprintw(5, 0, "Go! %s! \nHP:%d \nMoves: \n1. %s\n2. %s", pc_poke->get_species(), pc_poke->get_hp(), pc_poke->get_move(0), pc_poke->get_move(1));
    ...
    if (input == '1' || input == '2')
    {
      ...
      if (input == '2')
        move = 2;
      damage = pc_poke->get_dam(move, rand() % 16 + 85);
      if (pc_poke->get_acc(move) > rand() % 100)
        p->set_hp(-1 * damage);
      else
        damage = -1;
      clear();
      ...
      mvprintw(5, 0, "%s\nHP:%d\nMoves: \n1. %s\n2. %s", pc_poke->get_species(), pc_poke->get_hp(), pc_poke->get_move(0), pc_poke->get_move(1));
      if (damage == -1)
        mvprintw(10, 0, "%s missed", pc_poke->get_species());
      else
        mvprintw(10, 0, "%s did %d damage!", pc_poke->get_species(), damage);
      refresh();
      getch();
    }
    else if (input == 'b')
    ...
    if (itemPicked == 0)
    {
      world.pc.buddy[numPoke] = p;
      clear();
      mvprintw(0, 0, "You caught %s!", world.pc.buddy[1]->get_species());
      for (int i = 0; i < 6 && world.pc.buddy[i]; i++)
      {
        printw(", %s", world.pc.buddy[i]->get_species());
      }
    ...
    {
      pc_poke->set_hp(pc_poke->get_ogHP() * 0.5);
      itemPicked = -1;
    }
    else if (itemPicked == 2)
    {
      if (pc_poke->get_hp() + 20 <= pc_poke->get_ogHP())
      {
        pc_poke->set_hp(20);
      }
      else
      {
        int differenceHP = pc_poke->get_ogHP() - pc_poke->get_hp();
        pc_poke->set_hp(differenceHP);
      }
      itemPicked = -1;
    }
    ...
    {
      int odds = ((pc_poke->get_speed() * 32) / ((p->get_speed() / 4) % 256)) + 30 * runs;
      runs++;
      if (odds > rand() % 256)
        inBattle = 0;
    }

    damage = p->get_dam(rand() % 1 + 1, rand() % 16 + 85);
    if (p->get_acc(rand() % 1 + 1) > rand() % 100)
      pc_poke->set_hp(-1 * damage);
    else
      damage = -1;
    ...
    if (damage == -1)
      ...
      mvprintw(10, 0, "%s did %d damage!", p->get_species(), damage);
    refresh();
    getch();
    clear();
  } while (inBattle);
...
void io_backpack(int inBattle)
{
  ...
  while (inMenu)
  {
    ...
    // else if (input == 27)
    //   break;
    // mvprintw(i + 2, 5, "Enter a valid input");
    refresh();
  }
}
...
{
  clear();
  mvprintw(0, 0, "Your item: %s", world.pc.item[numItem].c_str());
  itemPicked = numItem;
  refresh();
  getch();
}
...
void io_trainer(npc *npc)
{
  ...
  pokemon *n_pokemon = npc->buddy[n_pokeIdx];
  pokemon *pc_poke = world.pc.buddy[p_pokeIdx];

  do
  {
    clear();
    if (n_pokemon->get_hp() == 0)
    {
      n_pokeIdx++;
      if (n_pokeIdx > numPoke)
        inBattle = 0;

      else if (npc->buddy[n_pokeIdx] == NULL)
        inBattle = 0;

      else
        n_pokemon = npc->buddy[n_pokeIdx];
    }

    if (pc_poke->get_hp() == 0)
    {
      p_pokeIdx++;
      if (p_pokeIdx > p_numPoke)
        inBattle = 0;

      else if (world.pc.buddy[p_pokeIdx] == NULL)
        inBattle = 0;

      else
        pc_poke = world.pc.buddy[p_pokeIdx];
    }

    mvprintw(0, 0, "Trainer has a %s!\nHP:%d", n_pokemon->get_species(), n_pokemon->get_hp());
    mvprintw(5, 0, "Go! %s!\nHP:%d\nMoves: \n1. %s\n2. %s", pc_poke->get_species(), pc_poke->get_hp(), pc_poke->get_move(0), pc_poke->get_move(1));
    mvprintw(10, 0, "select an action: \n(1) move 1\n(2) move 2\n(b) backpack");
    refresh();
    char input = getch();
    ...
    {
      if (pc_poke->get_hp() + 20 <= pc_poke->get_ogHP() && pc_poke->get_hp() != 0)
      {
        pc_poke->set_hp(20);
      }
      else
    ...
    damage = n_pokemon->get_dam(rand() % 1 + 1, rand() % 16 + 85);
    if (n_pokemon->get_acc(rand() % 1 + 1) > rand() % 100)
      pc_poke->set_hp(-1 * damage);
    else
      damage = -1;
    if (damage == -1)
      mvprintw(10, 0, "%s missed", n_pokemon->get_species());
    else
      mvprintw(10, 0, "%s did %d damage!", n_pokemon->get_species(), damage);
    refresh();
    getch();
    clear();
  } while (inBattle);
