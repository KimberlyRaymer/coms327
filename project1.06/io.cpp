...
// Pc is not need in this list, make all character type of Npc
...
uint32_t move_pc_dir(uint32_t input, pair_t dest)
{
  ...
  if ((world.cur_map->map[dest[dim_y]][dest[dim_x]] == ter_gate) && (input == 1 || input == 3 || input == 7 || input == 9))
  {
    /* Can't leave the map */
    return 1;
  }
...
void fly(pair_t pos)
{
  ...
  // null pc position from current map
  ...
  // prompt user
  mvprintw(0, 0, "To fly enter coordinate x");
  refresh();
  echo();
  curs_set(1);
  mvscanw(1, 0, "%d", &x);
  mvprintw(2, 0, "Now enter coordinate y");
  refresh();
  echo();
  curs_set(3);
  mvscanw(3, 0, "%d", &y);
  mvprintw(4, 0, "Have a safe trip!");
  refresh();
  echo();
