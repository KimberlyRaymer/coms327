...
//func for pc to exit map
//helper method to game_loop
void exitMap (pair_t pos) {
    //north gate
    if (pos[dim_y] == 0) {
      world.cur_idx[dim_y]--;
    }
    //south gate
    else if (pos[dim_y] == MAP_Y - 1) {
      world.cur_idx[dim_y]++;
    }
    //west gate
    else if (pos[dim_x] == 0) {
...
void game_loop()
{
  ...
  while (!world.quit) {
    ...
    pc = dynamic_cast<Pc *> (c);
    npc = dynamic_cast<Npc *> (c);

    move_func[npc ? npc->mtype : move_pc](c, d);

    world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
    if (pc && (d[dim_y] == 0 || d[dim_y] == MAP_Y - 1 || d[dim_x] == 0 || d[dim_x] == MAP_X - 1)) {
      exitMap(d);
      d[dim_y] = c->pos[dim_y];
      d[dim_x] = c->pos[dim_x];
    }
    ...
    c->next_turn += move_cost[npc ? npc->ctype : char_pc]
