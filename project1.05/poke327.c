...
/*Method that acts as a placeholder for battles*/
void pokeBattle(character_t *trainer)
{
  ...
  mvprintw(0, 0, "A battle ensues!");
  mvprintw(1, 0, "Press esc to leave battle");
  refresh();
  ...
  trainer->npc->defeated = 1;
  if (trainer->npc->ctype == char_hiker || trainer->npc->ctype == char_rival || trainer->npc->ctype == char_swimmer)
  {
    trainer->npc->mtype = move_explore;
  }
}
...
static void move_hiker_func(character_t *c, pair_t dest)
{
  ...
  for (i = base; i < 8 + base; i++)
  {
        ...
        c->pos[dim_y] + all_dirs[i & 0x7][dim_y] != MAP_Y - 1)
    {
      ...
      // pc cost == 0
      //check the 8 boxes surrounding
      if (world.hiker_dist[c->pos[dim_y] + all_dirs[i & 0x7][dim_y]][c->pos[dim_x] + all_dirs[i & 0x7][dim_x]] == 0)
      {
        // battle if encounter PC
        pokeBattle(c);
        break;
      }
    }
...
{
  terrain_type_t t;
  // STUCK PACER BUG fixed by modifying if statement conditions.
  dest[dim_x] = c->pos[dim_x];
  dest[dim_y] = c->pos[dim_y];
  ...
  if ((t != ter_path && t != ter_grass && t != ter_clearing && t != ter_mart && t != ter_center) ||
  ...
  // check if defeated, if not then trigger battle
  if (!c->npc->defeated && world.cur_map->cmap[c->pos[dim_y] + c->npc->dir[dim_y]][c->pos[dim_x] + c->npc->dir[dim_x]] == &world.pc)
  {
    // battle if encounter PC
    pokeBattle(c);
    return;
    ...
    if (((m->map[dest[dim_y] + dir[dim_y]]
         ...
         !((m->map[dest[dim_y] + dir[dim_y]]
           ...
           is_adjacent(((pair_t){dest[dim_x] + dir[dim_x],
                                 dest[dim_y] + dir[dim_y]}),
                       ter_water))) ||
        m->cmap[dest[dim_y] + dir[dim_y]][dest[dim_x] + dir[dim_x]])
    {
      rand_dir(c->npc->dir);
    }
...
/*Method to enter Pokemart or PokeCenter*/
void poke_M_C()
{
  ...
  while (in)
  {
    mvprintw(0, 0, "There is a sign, it says 'The building is underconstruction, please come back in a week'!");
    refresh();
...
void printTrainers () {
  ...
  // mvprintw(0, 0, "Trainers' List");

  // get pc location
  pc_pos[dim_y] = world.pc.pos[dim_y];
  pc_pos[dim_x] = world.pc.pos[dim_x];

// for loop through character map
    for (int y = 0; y < MAP_Y; y++)
    {
      for (int x = 0; x < MAP_X; x++)
      {
        ...
        if (world.cur_map->cmap[y][x] != NULL)
        {
          ...
          // the distance calculated for y and x
          npc_pos[dim_y] = pc_pos[dim_y] - npc_pos[dim_y];
          npc_pos[dim_x] = pc_pos[dim_x] - npc_pos[dim_x];

          // Print trainers symbol, distance y, distance x
          // N + E
          if (npc_pos[dim_y] > 0 && npc_pos[dim_x] > 0)
          {

            mvprintw(num_trainers, 0, "%c, %d north and %d east", world.cur_map->cmap[y][x]->symbol, npc_pos[dim_y], npc_pos[dim_x]);
            num_trainers++;
          }
          // N + W
          else if (npc_pos[dim_y] > 0 && npc_pos[dim_x] < 0)
          {

            mvprintw(num_trainers, 0, "%c, %d north and %d west", world.cur_map->cmap[y][x]->symbol, npc_pos[dim_y], npc_pos[dim_x]);
            num_trainers++;
          }
          // S + E
          else if (npc_pos[dim_y] < 0 && npc_pos[dim_x] > 0)
          {
            ...
            mvprintw(num_trainers, 0, "%c, %d south and %d east", world.cur_map->cmap[y][x]->symbol, npc_pos[dim_y], npc_pos[dim_x]);
            num_trainers++;
          ...
          else if (npc_pos[dim_y] < 0 && npc_pos[dim_x] < 0)
          {
            ...
            mvprintw(num_trainers, 0, "%c, %d south and %d west", world.cur_map->cmap[y][x]->symbol, npc_pos[dim_y], npc_pos[dim_x]);
            num_trainers++;
          }
        }
      }
...
/*Method to list trainers: symbol, pos y (n or s) and pos x (w or e)*/
void list_trainers(character_t trainers[], int count, int input)
{

  // clear to show the trainers' list
  clear();
  refresh();
  move(0, 0);
  ...
  while (1)
  {
    ...
    //Total trainers in map - 8 (the eight being the fix number of trainers to be displayed)
    // Setting the list to a fix number displaying 8 mvprintw statements
    if (numTrainers > 8)
    {
      // cmd 'escape' or '27' ascii
      ...
      case KEY_UP:
        ...
        break;
      case KEY_DOWN:
        if (listDifference < (numTrainers - 8)) {
          listDifference++;
        }
        break;
...
/*Counter number of trainers*/
int nTrainers(int input) {

  //count how many trainers in map
  numTrainers = 0;
  for (int y = 0; y < MAP_Y; y++)
  {
    for (int x = 0; x < MAP_X; x++)
  ...
  //create an array list of trainers
  ...
  for (int y = 0; y < MAP_Y; y++)
  {
    for (int x = 0; x < MAP_X; x++)
    {
      //if there's a npc and it is not the player
      if (world.cur_map->cmap[y][x] != NULL && world.cur_map->cmap[y][x]->symbol != '@')
      { 
        trainers[index].symbol = world.cur_map->cmap[y][x]->symbol;
        trainers[index].pos[dim_y] = y;
        trainers[index].pos[dim_x] = x;

        index++;
      }
    }
  }
  // list_trainers(trainers, input, numTrainers);
...
int move_PC(map_t *m)
{
  ...
  pair_t newPos;
  newPos[dim_y] = world.pc.pos[dim_y];
  newPos[dim_x] = world.pc.pos[dim_x];
  ...
  // 7 or y: Attempt to move PC on cell to the upper left
  case '7':
  case 'y':
    newPos[dim_y]--;
    newPos[dim_x]--;
  ...
  // 6 or l (L): Attempt to move PC one cell to the right
  case '6':
  case 'l':
    newPos[dim_x]++;
    break;
  ...
  // escape (ascii 27): When displaying trainer list, return to character control
  case 27:
    break;
  ...
  // default: error cmd
  default:
    mvprintw(0, 0, "Invalid command, please enter a valid key");
  }

  if (world.cur_map->cmap[newPos[dim_y]][newPos[dim_x]])
  {
    // if this is the npc
    if (world.cur_map->cmap[newPos[dim_y]][newPos[dim_x]]->npc && !world.cur_map->cmap[newPos[dim_y]][newPos[dim_x]]->npc->defeated)
    {
      // battle
      pokeBattle(world.cur_map->cmap[newPos[dim_y]][newPos[dim_x]]);
    }
    // keep pc in old position as it is a battle, but hiker, rival, swimmer will move else where
    newPos[dim_y] = world.pc.pos[dim_y];
    newPos[dim_x] = world.pc.pos[dim_x];
  }

  //if the move cost for pc's next position is INT_MAX, keep pc in it's original position
  // hit an unmovable object, exit game
  if (move_cost[char_pc][world.cur_map->map[newPos[dim_y]][newPos[dim_x]]] == INT_MAX)
  {
    newPos[dim_y] = world.pc.pos[dim_y];
    newPos[dim_x] = world.pc.pos[dim_x];
  }
  ...
  world.pc.pos[dim_y] = newPos[dim_y];
  world.pc.pos[dim_x] = newPos[dim_x];

  world.cur_map->cmap[newPos[dim_y]][newPos[dim_x]] = &world.pc;
